...
`IScannerMetadataEnricher` runs after read-only WIA enumeration and before normalization. Injectable PnP and allowlisted registry providers correlate conservatively, isolate failures and timeouts, and return only bounded metadata plus SHA-256 identifiers. They never write the registry, open a scan session, load vendor code, or change the stable scanner identity. Vendor SDK support is an interface boundary only.
